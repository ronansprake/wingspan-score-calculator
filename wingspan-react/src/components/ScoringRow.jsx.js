...
function PlayerInitialControl({label, player, row, currentScores, changeScore, isDisabled, ...props}) {
  ...
  return (
    <div className="counter player-{player}">
      ...
      <input onChange={(e) => {changeScore(row,player,(e.target.value))}} id={"player-" + player + "-" + label} onContextMenu={(e)=> e.preventDefault()} onFocus={(e)=> { e.target.select(); e.preventDefault(); e.stopPropagation(); return false; }} type="number" className="js-score" {...props} />
